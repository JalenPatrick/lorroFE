...
function (_Component) {
  ...
  function results() {
      ...
      if (!_this.state.transcribeDone) {
        axios__WEBPACK_IMPORTED_MODULE_21___default.a.post(transcribe_url, transcribe).then(function (response) {
          ...
          if (response.data.status === 'COMPLETED') {
            ...
            console.log(_this.state.transcribeDone);
          }
        });
      }
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "getTranscribe", function (trans_url, wasCalled) {
      // console.log('got here', trans_url);
      if (!wasCalled) {
  ...
  _createClass(results, [{
    ...
    value: function render() {
      ...
      if (this.state.transcribeDone && !wasCalled) {
        transcribe_result = this.getTranscribe(this.state.transcribeHere, wasCalled);
        determineWords = this.matchingWords(wordMatching, wasCalled);
...
//# sourceMappingURL=results.js.b1d2cc84e17d5958133f.hot-update.js.map